...
describe('Videogame API', () => {
    it('POST', async () => {
        return axios.post(`${API_URL}?user=${TEST_USER}`, testVideogame).then((response) => {
    ...
    it('GET', async () => {
        return axios.get(`${API_URL}?user=${TEST_USER}`).then((response) => {
            expect(response.status).toBe(200);
    ...
    it('PUT', async () => {
        ...
        return axios.put(`${API_URL}?user=${TEST_USER}`, updatedData).then((response) => {
            expect(response.status).toBe(200);
            expect(response.data).toMatchObject({ success: true });
        });
    }
    ...
    it('DELETE', async () => {
        return axios.delete(`${API_URL}?user=${TEST_USER}&id=999`).then((response) => {
            expect(response.status).toBe(200);
            expect(response.data).toMatchObject({ success: true });
        });
    }
